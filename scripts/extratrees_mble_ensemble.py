"""
ExtraTrees + GPS + mBle 피처 앙상블
- parquet v2 + GPS 19개 + mBle 25개
- Optuna 100 trials/target, 10 seeds
출력: submission/extratrees_mble_prob.csv (clip 0.1~0.9)
"""
...
def main():
    ...
    print("=== mBle 피처 집계 중 ===")
    mble_feat = build_mble()
    parquet_feat = parquet_feat.merge(mble_feat, on=["subject_id", "date"], how="left")

    extra_cols = (
        [c for c in gps_feat.columns  if c not in ("subject_id", "date")] +
        [c for c in mble_feat.columns if c not in ("subject_id", "date")]
    )
    nan_rate = parquet_feat[extra_cols].isna().mean()
    print(f"  GPS+mBle NaN 비율 (평균): {nan_rate.mean():.3f}")
    print()

    print(f"=== ExtraTrees + GPS + mBle 앙상블 ({N_TRIALS} trials) ===\n")
    ...
    out_path = SUBMISSION_DIR / "extratrees_mble_prob.csv"
