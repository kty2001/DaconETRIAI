"""
ExtraTrees v5 + GPS Slim 85% 앙상블
피처 구성 (v5):
  - parquet v5: v2 + wLight + mBle + mWifi
  - GPS (mGps)
  - DOW 편차: 개인별 요일 평균 대비 편차 (step/hr/light/screen/gps/wifi/ble)
- Phase 0: 중요도 계산 (extratrees_gps params 활용, v5 피처 기준)
- Phase 1: Optuna 100 trials (extratrees_v5_gps_slim85)
- Phase 2: 10 seeds 앙상블
출력: submission/extratrees_v5_gps_slim85_prob.csv (clip 0.1~0.9)
"""
...
from parquet_features_v5 import build_all as build_parquet_features
...
ET_KEY_SLIM_V5  = "extratrees_v5_gps_slim85"
N_TRIALS        = 100
IMP_COVERAGE    = 0.85
...
DOW_BASE_FEATURES = [
    "pedo_step_sum", "pedo_calories_sum",
    "act_active_ratio", "act_active_cnt",
    "hr_mean", "hr_std", "hr_sleep_rmssd",
    "wlight_daily_mean", "wlight_sleep_dark_ratio", "wlight_presleep_to_sleep_drop",
    "light_mean",
    "screen_on_ratio", "screen_on_count",
    "gps_speed_mean", "gps_moving_ratio", "gps_place_entropy",
    "gps_n_places", "gps_home_ratio",
    "wifi_entropy", "wifi_n_unique_daily", "wifi_home_ratio",
...
def compute_dow_deviations(parquet_feat: pd.DataFrame, train: pd.DataFrame) -> pd.DataFrame:
    """
    훈련 날짜 기준으로 subject별 요일 평균을 계산하고
    전체 parquet 피처에 편차/평균 피처를 추가한 df를 반환 (subject_id, date 키)
    """
    pf = parquet_feat.copy()
    pf["_dow"] = pd.to_datetime(pf["date"]).dt.dayofweek

    train_pairs = set(zip(train["subject_id"], train["lifelog_date"]))
    pf["_is_train"] = [
        (sid, d) in train_pairs
        for sid, d in zip(pf["subject_id"], pf["date"])
    ]

    dow_cols = [f for f in DOW_BASE_FEATURES if f in pf.columns]
    train_pf = pf[pf["_is_train"]]
    dow_means = (
        train_pf.groupby(["subject_id", "_dow"])[dow_cols]
        .mean()
        .reset_index()
        .rename(columns={f: f"__dm_{f}" for f in dow_cols})
    )

    pf = pf.merge(dow_means, on=["subject_id", "_dow"], how="left")

    result = parquet_feat[["subject_id", "date"]].copy()
    for f in dow_cols:
        dm_col = f"__dm_{f}"
        if dm_col in pf.columns:
            result[f"{f}_dow_dev"]  = (pf[f] - pf[dm_col]).values
            result[f"{f}_dow_mean"] = pf[dm_col].values

    n_new = len(result.columns) - 2
    print(f"  DOW 편차 피처: {n_new}개 생성 (기준 피처 {len(dow_cols)}개)")
    return result
...
def compute_importance(train, parquet_feat, le, sensor_cols, fold_label_feats, best_gps):
    ...
    # 새 피처 상위 10개 확인
    new_feat_imp = combined[combined.index.str.contains("wlight|ble_|wifi_|_dow_", na=False)]
    if len(new_feat_imp) > 0:
        print("  [신규 피처 상위 10개]")
        for feat, row in new_feat_imp.head(10).iterrows():
            print(f"    {feat}: {row['mean_imp']:.5f}")
...
def train_and_predict(train, test, parquet_feat):
    ...
    cached_et = load_params(ET_KEY_SLIM_V5)
    if cached_et:
        print(f"=== Phase 1: 저장된 params 로드 ({ET_KEY_SLIM_V5}) ===")
        ...
        print(f"=== Phase 1: ExtraTrees Optuna v5 slim85 ({N_TRIALS} trials/target) ===")
        best_et = {}
        for t in TARGETS:
            y = train[t].values
            ...
            best_et[t] = bp
            ref_ll = ET_LL_REF.get(t, "-")
            print(f"  {t}: LL={study.best_value:.4f} (ref: {ref_ll}) | "
                  f"n_est={bp['n_estimators']}, depth={bp['max_depth']}, "
                  f"max_feat={bp['max_features']:.4f}")
        print()
        save_params(ET_KEY_SLIM_V5, best_et)

    print(f"=== Phase 2: ExtraTrees v5 slim85 앙상블 ({n_seeds} seeds) ===")
    ...
    print()
    print(f"{'타깃':<5}  {'F1':>6}  {'OOF LL':>8}  {'ref LL':>8}  {'개선':>7}")
    print("-" * 44)
    lls_final = []
    for t in TARGETS:
    ...
    avg_ref = np.mean(list(ET_LL_REF.values()))
...
def main():
    ...
    print("=== DOW 편차 피처 계산 중 ===")
    dow_feat = compute_dow_deviations(parquet_feat, train)
    parquet_feat = parquet_feat.merge(dow_feat, on=["subject_id", "date"], how="left")
    total_cols = len([c for c in parquet_feat.columns if c not in ("subject_id", "date")])
    print(f"  총 parquet 피처: {total_cols}개\n")

    print(f"=== ExtraTrees v5 GPS Slim {IMP_COVERAGE*100:.0f}% ===\n")
    ...
    out_path = SUBMISSION_DIR / "extratrees_v5_gps_slim85_prob.csv"
