"""
LightGBM 멀티 시드 평균: 10개 시드 × GroupKFold(10) LOSO
subsample=0.8, colsample_bytree=0.8 추가로 시드 간 다양성 확보
OOF 예측 및 test 예측 모두 시드별 평균
출력: submission/lgbm_multiseed_prob.csv (clip 0.1~0.9)
"""
...
def train_and_predict(train, test, parquet_feat):
    ...
    # test z-score용 full stats 사전 계산
    ...
    for seed_i, seed in enumerate(SEEDS):
        ...
        # 시드별 중간 성능 출력 (누적 평균 기준)
        f1s, lls = [], []
        for t in TARGETS:
            cur_oof = oof_accum[t] * n_seeds / (seed_i + 1)  # 현재까지 평균
...
def main():
    ...
    print(f"=== LightGBM 멀티 시드 ({len(SEEDS)}개) + z-score ===\n")
    ...
    out_path = SUBMISSION_DIR / "lgbm_multiseed_prob.csv"
