"""
LightGBM + Optuna + mUsageStats 선별 추가
3가지 방법론 공통 상위 3개만 유지: usage_apps_morning, usage_apps_afternoon, usage_ms_total
나머지 mUsageStats 10개 제거 (전체 추가 시 과적합 방지)
N_TRIALS=30, 출력: submission/lgbm_usage_select_prob.csv
"""
...
# 하위 mUsageStats 10개 제거 (상위 3개 유지: usage_apps_morning, usage_apps_afternoon, usage_ms_total)
DROP_USAGE = [
    "usage_ms_morning", "usage_ms_afternoon", "usage_ms_evening",
    "usage_ms_presleep", "usage_ms_sleep",
    "usage_apps_evening", "usage_apps_presleep", "usage_apps_sleep",
...
def precompute_fold_features(train, test, parquet_feat, le, sensor_cols, fold_label_feats, label_feat_test, full_stats):
    ...
    # 타깃별 subj_mean 제거 + 하위 mUsageStats 제거
    fold_by_target = {}
    te_by_target   = {}
    for t in TARGETS:
...
def main():
    ...
    print(f"=== LightGBM + Optuna ({N_TRIALS} trials) + mUsageStats 선별(3개) + 멀티 시드 ({len(SEEDS)}개) ===\n")
    ...
    out_path = SUBMISSION_DIR / "lgbm_usage_select_prob.csv"
