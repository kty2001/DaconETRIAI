"""
ET GPS Slim 80% + Transductive Z-score + 피험자별 개인 모델 블렌딩

구조:
  전체 모델 (Global):  전체 450행 학습 (ExtraTrees, best_slim params, 10 seeds)
  개인 모델 (Personal): 피험자별 자신의 데이터만 학습 (shallow ET, depth=3, 10 seeds)
  블렌드: alpha * personal + (1-alpha) * global
  alpha: WS OOF val log-loss 최소화로 전역 최적화
  편향 보정: 블렌드된 WS OOF 예측으로 피험자별 로짓 편향 보정

출력: submission/et_gps_slim80_personal_blend_prob.csv
"""
...
# 개인 모델: 피험자 1명 데이터(33~57행)에 맞게 얕게 설정
PERS_N_EST    = 50
PERS_DEPTH    = 3
PERS_MAX_FEAT = 0.3
PERS_MIN_LEAF = 2
...
def blend_probs(global_p, pers_p, alpha):
    """alpha: 개인 모델 가중치 (0=전체 모델만, 1=개인 모델만)"""
...
def main():
    ...
    print("=== ET GPS Slim 80% + Transductive + Global+Personal 블렌드 ===\n")
    ...
    print("\n=== Phase 3: 블렌드 alpha 최적화 (WS OOF val 전체) ===")
    ...
    # 타깃별 WS OOF LL
    print("\n  타깃별 WS OOF LL (블렌드 alpha={:.4f}):".format(best_alpha))
    ws_ll_blend = {}
    for t in TARGETS:
        y_v = train_reset.loc[all_val_pos, t].values
        g_v = np.array([ws_oof_global[t][p] for p in all_val_pos])
    ...
    out_path = SUBMISSION_DIR / "et_gps_slim80_personal_blend_prob.csv"
    result_prob.to_csv(out_path, index=False)
    print(f"\n저장 완료: {out_path}")
    print(f"WS OOF 블렌드 평균 LL: {best_ll:.4f}  (alpha={best_alpha:.4f})")
    print("참고: 기존 et_gps_slim80_transductive Public 0.6020 대비 비교 필요.")
