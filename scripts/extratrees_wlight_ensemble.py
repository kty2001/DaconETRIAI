"""
ExtraTrees + GPS + wLight 피처 앙상블
- parquet v2 + GPS 19개 + wLight 16개
- Optuna 100 trials/target, 10 seeds
- GPS/wLight 피처도 subject z-score 정규화
출력: submission/extratrees_wlight_prob.csv (clip 0.1~0.9)
"""
...
def main():
    ...
    print("=== wLight 피처 집계 중 ===")
    wlight_feat = build_wlight()
    parquet_feat = parquet_feat.merge(wlight_feat, on=["subject_id", "date"], how="left")

    extra_cols = (
        [c for c in gps_feat.columns    if c not in ("subject_id", "date")] +
        [c for c in wlight_feat.columns if c not in ("subject_id", "date")]
    )
    nan_rate = parquet_feat[extra_cols].isna().mean()
    print(f"  GPS+wLight NaN 비율 (평균): {nan_rate.mean():.3f}")
    print()

    print(f"=== ExtraTrees + GPS + wLight 앙상블 ({N_TRIALS} trials) ===\n")
    ...
    out_path = SUBMISSION_DIR / "extratrees_wlight_prob.csv"
