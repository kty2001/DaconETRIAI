...
출력: submission/hgb_et_ensemble_prob.csv (clip 0.1~0.9)
"""
...
def train_and_predict(train, test, parquet_feat):
    ...
    # ── Phase 2: 멀티 시드 앙상블 ────────────────────────────────────────────
    n_seeds = len(SEEDS)
    print(f"=== Phase 2: HGB+ET 앙상블 ({n_seeds} seeds) ===")
    ...
    print()
    print(f"{'타깃':<5}  {'앙상블 F1':>8}  {'앙상블 LL':>9}  {'HGB LL':>7}  {'ET LL':>7}  {'기준 F1':>7}")
    print("-" * 52)
    et_lls  = {"Q1":0.6998,"Q2":0.6478,"Q3":0.6443,"S1":0.6161,"S2":0.6161,"S3":0.6124,"S4":0.6870}
    hgb_lls = {"Q1":0.6988,"Q2":0.6577,"Q3":0.6336,"S1":0.6141,"S2":0.6193,"S3":0.6230,"S4":0.6795}
    for t in TARGETS:
        f1 = f1_score(train[t].values, (ens_oof[t] > 0.5).astype(int))
        ll = log_loss(train[t].values, ens_oof[t])
        print(f"{t:<5}  {f1:>8.3f}  {ll:>9.4f}  {hgb_lls[t]:>7.4f}  {et_lls[t]:>7.4f}  {BASELINE_F1[t]:>7.3f}")
    ...
    print(f"{'평균':<5}  {avg_f1:>8.3f}  {avg_ll:>9.4f}  {'0.6466':>7}  {'0.6462':>7}")
