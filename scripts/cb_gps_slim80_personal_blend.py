"""
CB GPS Slim 80% Personal Blend (WS OOF Optuna)

Global: CatBoost Classifier (subject_enc categorical, WS OOF Optuna 30 trials/target)
Personal: ExtraTrees depth=2

CatBoost 고유 강점:
  - subject_enc (66 classes)를 네이티브 카테고리로 처리 -> 피험자별 효과 자동 학습
  - Ordered boosting -> 소규모 데이터 과적합 방지

출력:
  submission/cb_gps_slim80_personal_blend_prob.csv
"""
...
# Personal model (검증된 파라미터)
PERS_N_EST    = 50
PERS_DEPTH    = 2
PERS_MAX_FEAT = 0.5
PERS_MIN_LEAF = 3

PARAMS_KEY = "cb_gps_slim80_ws"
...
def get_cat_idx(cols):
    cols_list = list(cols)
    return [cols_list.index("subject_enc")] if "subject_enc" in cols_list else []
...
def main():
    ...
    for sid in subjects:
        ...
        for t in TARGETS:
            ...
            if len(np.unique(y_tr)) >= 2:
                glb = CatBoostClassifier(
                    **{**CB_FIXED, **cb_params[t], "random_seed": WS_SEED}
                )
                glb.fit(X_tr_f, y_tr, cat_features=cat_idx)
    ...
    # ── Phase 6: Global CB (10 seeds) + Personal ET (10 seeds) ──
    print(f"\n=== Phase 6: Global CB model ({len(SEEDS)} seeds) ===", flush=True)
    cb_test_global = {t: np.zeros(len(sample)) for t in TARGETS}
    for seed in SEEDS:
        for t in TARGETS:
            cols_t  = ws_cols[t]
            cat_idx = get_cat_idx(cols_t)
            ...
            model = CatBoostClassifier(**{**CB_FIXED, **cb_params[t], "random_seed": seed})
            model.fit(X_all_t.fillna(all_med), train_reset[t].values, cat_features=cat_idx)
            cb_test_global[t] += model.predict_proba(X_te_t.fillna(all_med))[:, 1] / len(SEEDS)
        print(f"  seed {seed} done", flush=True)

    print(f"\n=== Phase 6b: Personal ET ({len(SEEDS)} seeds) ===", flush=True)
    cb_test_pers = {t: np.zeros(len(sample)) for t in TARGETS}
    for sid in subjects:
        ...
        for t in TARGETS:
            ...
            cb_test_pers[t][mask_te] = preds_sid
    ...
    cb_test_blend = {t: blend_probs(cb_test_global[t], cb_test_pers[t], best_alpha)
                     for t in TARGETS}
    ...
    cb_test_cal = {t: cb_test_blend[t].copy() for t in TARGETS}
    for sid in subjects:
        mask_te = (sample["subject_id"] == sid).values
        if mask_te.sum() == 0:
            continue
        for t in TARGETS:
            cb_test_cal[t][mask_te] = apply_logit_bias(
                cb_test_blend[t][mask_te], biases[sid][t]
            )

    result_prob = sample[["subject_id", "sleep_date", "lifelog_date"]].copy()
    for t in TARGETS:
        result_prob[t] = cb_test_cal[t].clip(0.1, 0.9)
    ...
    out_path = SUBMISSION_DIR / "cb_gps_slim80_personal_blend_prob.csv"
