"""
ET GPS + Rolling 피처 + Slim 80% + Transductive + Personal Blend

et_gps_slim80_personal_blend.py 대비 변경:
  - rolling_features.build_rolling_features() 피처 추가
    (기준 19개 피처의 3/7일 MA + delta3 = 최대 57개 신규 피처)
  - parquet_feat에 rolling 피처 병합 후 동일 파이프라인 적용
  - wHr 변동성 피처 미포함 (ET slim에서 효과 없음 확인)

출력: submission/et_gps_rolling_slim80_personal_blend_prob.csv
"""
...
def main():
    ...
    print("=== parquet v2 + GPS + Rolling 피처 집계 중 ===")
    ...
    print(f"  rolling 병합 후: {parquet_feat.shape[1]}개 컬럼")
    print()
    ...
    print(f"  공통 피처: {len(combined)}개 -> slim {IMP_COVERAGE*100:.0f}%: {len(keep_feats)}개")
    ...
    print(f"  rolling 피처 선택: {len(selected_rolling)}/{len(rolling_cols)}개")
    if selected_rolling:
        print(f"    선택됨: {selected_rolling[:10]}{'...' if len(selected_rolling) > 10 else ''}")
    ...
    for sid in subjects:
        ...
        print(f"  {sid}: {n_dates}일 (val={n_val}일)")

    # Phase 3: 블렌드 alpha 최적화
    print("\n=== Phase 3: 블렌드 alpha 최적화 ===")
    ...
    best_alpha_grid = alphas[int(np.argmin(grid_ll))]
    res = minimize_scalar(blend_logloss, bounds=(
        max(0.0, best_alpha_grid - 0.05),
        min(1.0, best_alpha_grid + 0.05)
    ), method="bounded")
    best_alpha = float(res.x)
    best_ll    = float(res.fun)

    print(f"  alpha grid: {best_alpha_grid:.3f}, fine-tuned: {best_alpha:.4f}")
    print(f"  WS OOF 블렌드 LL (alpha={best_alpha:.4f}): {best_ll:.4f}")
    print(f"  WS OOF 전체 모델만 (alpha=0.0): {blend_logloss(0.0):.4f}")
    ...
    for t in TARGETS:
        ...
        ws_ll_blend[t] = ll
        print(f"    {t}: {ll:.4f}")
    print(f"    평균: {np.mean(list(ws_ll_blend.values())):.4f}")
    ...
    # Phase 6: 블렌드
    print(f"\n=== Phase 6: 블렌드 (alpha={best_alpha:.4f}) ===")
    et_test_blend = {}
    for t in TARGETS:
        et_test_blend[t] = blend_probs(et_test_global[t], et_test_pers[t], best_alpha)

    # Phase 7: WS OOF 기반 편향 보정
    print(f"\n=== Phase 7: WS OOF 기반 편향 보정 (REG={CALIB_REG}) ===")
    biases = {}
    for sid in subjects:
        val_pos = ws_splits[sid]["val_pos"]
        biases[sid] = {}
    ...
    print(f"\n{'subject':<8}  " + "  ".join(f"{t:>7}" for t in TARGETS) + "  avg|bias|")
    ...
    out_path = SUBMISSION_DIR / "et_gps_rolling_slim80_personal_blend_prob.csv"
    result_prob.to_csv(out_path, index=False)
    print(f"\n저장 완료: {out_path}")
    print(f"WS OOF 블렌드 평균 LL: {best_ll:.4f}  (alpha={best_alpha:.4f})")
