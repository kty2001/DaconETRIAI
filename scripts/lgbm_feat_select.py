"""
LightGBM + Optuna + 피처 선택
Permutation Importance 음수 피처(79개) 제거 후 성능 검증
lgbm_optuna.py 동일 파이프라인, N_TRIALS=30
출력: submission/lgbm_feat_select_prob.csv (clip 0.1~0.9)
"""
...
# Permutation Importance 음수 피처 (전체 타깃 평균 기준)
DROP_FEATURES = [
    'act_active_cnt', 'act_active_ratio', 'act_ratio_afternoon', 'act_ratio_evening',
    'act_ratio_morning', 'act_ratio_sleep', 'act_still_ratio',
    'amb_Breathing', 'amb_Music', 'amb_Other', 'amb_Snoring', 'amb_Television',
    'amb_White_noise', 'amb_night_Breathing', 'amb_night_Computer_keyboard',
    'amb_night_Inside_small_room', 'amb_night_Silence', 'amb_night_Television',
    'amb_night_White_noise',
    'hr_mean', 'hr_mean_afternoon', 'hr_mean_evening', 'hr_mean_morning',
    'hr_mean_presleep', 'hr_mean_sleep', 'hr_std_afternoon', 'hr_std_sleep',
    'is_weekend',
    'lag1_Q2', 'lag1_S2', 'lag1_S3', 'lag1_S4',
    'lag2_Q3', 'lag2_S2', 'lag2_S3', 'lag2_S4',
    'lag7_Q1', 'lag7_Q2', 'lag7_S1', 'lag7_S3',
    'light_mean', 'light_mean_afternoon', 'light_mean_evening',
    'light_mean_presleep', 'light_mean_sleep',
    'month',
    'pedo_calories_sum', 'pedo_distance_sum', 'pedo_speed_mean',
    'pedo_step_afternoon', 'pedo_step_evening', 'pedo_step_morning',
    'pedo_step_sleep', 'pedo_step_sum',
    'roll14_Q1', 'roll14_S2', 'roll14_S3', 'roll14_S4',
    'roll3_Q3', 'roll3_S1', 'roll3_S3', 'roll3_S4',
    'roll7_Q1', 'roll7_S1', 'roll7_S2', 'roll7_S3', 'roll7_S4',
    'screen_on_count', 'screen_on_ratio', 'screen_ratio_evening', 'screen_ratio_morning',
    'usage_apps_evening', 'usage_ms_evening', 'usage_ms_morning',
    'usage_ms_presleep', 'usage_presleep_ratio', 'usage_sleep_ratio',
]
...
def precompute_fold_features(train, test, parquet_feat, le, sensor_cols, fold_label_feats, label_feat_test, full_stats):
    ...
    # 타깃별 subj_mean 제거 + DROP_FEATURES 적용
    fold_by_target = {}
    te_by_target   = {}
    for t in TARGETS:
        drop_cols = [f"subj_mean_{t}"] + DROP_FEATURES
        fold_by_target[t] = [
            (tr_idx, val_idx,
...
def main():
    ...
    print(f"=== LightGBM + Optuna ({N_TRIALS} trials/target) + 피처 선택 ({135 - len(DROP_FEATURES)}개) + 멀티 시드 ({len(SEEDS)}개) ===\n")
    ...
    out_path = SUBMISSION_DIR / "lgbm_feat_select_prob.csv"
