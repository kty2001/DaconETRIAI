"""
ExtraTrees + v3 피처 + GPS Slim 앙상블
- parquet v3 (wHr심층/mScreen/mAmbience구간분리/wPedo심층) + GPS 19개
- Phase 0: Feature Importance 계산 (extratrees_gps params로 대체 or 새 학습)
- Phase 1: Optuna 100 trials (extratrees_v3gps_slim)
- Phase 2: 10 seeds 앙상블
출력: submission/extratrees_v3gps_slim_prob.csv (clip 0.1~0.9)
"""
...
ET_KEY_SLIM = "extratrees_v3gps_slim"
N_TRIALS    = 100
IMP_COVERAGE = 0.90
...
def compute_importance(train, parquet_feat, le, sensor_cols, fold_label_feats):
    """초기 params로 Feature Importance 계산 (seed=42, 고정 합리적 params)"""
    init_params = {
        "n_estimators": 500, "max_depth": 5, "max_features": 0.3,
    ...
    for tr_idx, val_idx, train_fold, val_fold, lf_tr, lf_val in fold_label_feats:
        ...
        for t in TARGETS:
            ...
            y = train[t].values
            model = ExtraTreesClassifier(**init_params)
...
def train_and_predict(train, test, parquet_feat):
    ...
    keep_feats = compute_importance(train, parquet_feat, le, sensor_cols, fold_label_feats)
    print()
    ...
    if cached_et:
        ...
        for t in TARGETS:
            ...
            print(f"  {t}: LL={study.best_value:.4f} (slim: {ref_ll}) | "
                  f"n_est={bp['n_estimators']}, depth={bp['max_depth']}, "
                  f"max_feat={bp['max_features']:.4f}")
        print()
        save_params(ET_KEY_SLIM, best_et)

    print(f"=== Phase 2: ExtraTrees v3+GPS slim 앙상블 ({n_seeds} seeds) ===")
    ...
    print()
    print(f"{'타깃':<5}  {'F1':>6}  {'OOF LL':>8}  {'slim LL':>8}  {'개선':>7}")
    print("-" * 44)
    lls_final = []
    for t in TARGETS:
...
def main():
    ...
    print("=== parquet 피처 v3 + GPS 집계 중 ===")
    parquet_feat = build_parquet_features()
    gps_feat     = build_gps()
    parquet_feat = parquet_feat.merge(gps_feat, on=["subject_id", "date"], how="left")
    print(f"  v3+GPS 피처 합계: {len(parquet_feat.columns)-2}개\n")

    print(f"=== ExtraTrees v3+GPS Slim ({IMP_COVERAGE*100:.0f}% coverage) ===\n")
    ...
    out_path = SUBMISSION_DIR / "extratrees_v3gps_slim_prob.csv"
