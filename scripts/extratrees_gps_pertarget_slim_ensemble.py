"""
ExtraTrees + GPS 타깃별 Feature Selection 앙상블
- 전체 평균 중요도 대신 타깃별 중요도로 각각 피처 선택
- 각 타깃이 자신에게 가장 중요한 피처 집합만 사용
- Phase 0: 타깃별 Feature Importance 계산
- Phase 1: Optuna 100 trials (타깃별 피처 집합으로)
- Phase 2: 10 seeds 앙상블
출력: submission/extratrees_gps_pertarget_slim_prob.csv (clip 0.1~0.9)
"""
...
ET_KEY_SLIM = "extratrees_gps_pertarget_slim"
N_TRIALS    = 100
IMP_COVERAGE = 0.85   # 타깃별 상위 85% 피처 유지
...
def compute_per_target_importance(train, parquet_feat, le, sensor_cols,
                                  fold_label_feats, best_gps):
    """타깃별로 독립적인 Feature Importance 계산"""
    importance_dict = {t: [] for t in TARGETS}
    feature_names_ref = {t: None for t in TARGETS}
    ...
    for tr_idx, val_idx, train_fold, val_fold, lf_tr, lf_val in fold_label_feats:
        ...
        for t in TARGETS:
            ...
            if feature_names_ref[t] is None:
    ...
    # 타깃별 keep 피처 계산
    keep_feats_per_target = {}
    for t in TARGETS:
        imp = np.mean(importance_dict[t], axis=0)
        imp_series = pd.Series(imp, index=feature_names_ref[t]).sort_values(ascending=False)
        total = imp_series.sum()
        cumsum = imp_series.cumsum() / total
        keep = imp_series[cumsum <= IMP_COVERAGE].index.tolist()
        keep_feats_per_target[t] = keep
        print(f"  {t}: {len(imp_series)}개 중 {len(keep)}개 유지 "
              f"({len(imp_series)-len(keep)}개 제거)")

    return keep_feats_per_target
...
def precompute_fold_features(train, test, parquet_feat, le, sensor_cols,
    ...
    for t in TARGETS:
        drop_cols = [f"subj_mean_{t}"] + DROP_USAGE
        keep = keep_feats_per_target[t]

        # 첫 번째 폴드 기준으로 컬럼 순서 확정
        first_tr  = fold_base[0][2].drop(columns=drop_cols, errors="ignore")
        slim_cols = [c for c in first_tr.columns if c in set(keep)]
...
def train_and_predict(train, test, parquet_feat):
    ...
    print(f"=== Phase 0: 타깃별 Feature Importance 계산 ({IMP_COVERAGE*100:.0f}% coverage) ===")
    keep_feats_per_target = compute_per_target_importance(
        train, parquet_feat, le, sensor_cols, fold_label_feats, best_gps
    )
    print()

    print("폴드 피처 사전 계산 중 (타깃별 slim)...")
    fold_by_target, te_by_target = precompute_fold_features(
        train, test, parquet_feat, le, sensor_cols,
        fold_label_feats, label_feat_test, full_stats, keep_feats_per_target,
    )
    for t in TARGETS:
        n = fold_by_target[t][0][2].shape[1]
        print(f"  {t}: {n}개 피처")
    print()
    ...
    if cached_et:
        ...
        print(f"=== Phase 1: ExtraTrees Optuna 타깃별 ({N_TRIALS} trials/target) ===")
        best_et = {}
        for t in TARGETS:
            y = train[t].values
            ...
            best_et[t] = bp
            ref_ll = ET_LL_SLIM85.get(t, "-")
            print(f"  {t}: LL={study.best_value:.4f} (slim85: {ref_ll}) | "
                  f"n_est={bp['n_estimators']}, depth={bp['max_depth']}, "
                  f"max_feat={bp['max_features']:.4f}")
        print()
        save_params(ET_KEY_SLIM, best_et)

    print(f"=== Phase 2: ExtraTrees 타깃별 slim 앙상블 ({n_seeds} seeds) ===")
    ...
    avg_ref = np.mean(list(ET_LL_SLIM85.values()))
...
def main():
    ...
    print(f"=== ExtraTrees GPS 타깃별 Slim ({IMP_COVERAGE*100:.0f}%) ===\n")
    ...
    out_path = SUBMISSION_DIR / "extratrees_gps_pertarget_slim_prob.csv"
