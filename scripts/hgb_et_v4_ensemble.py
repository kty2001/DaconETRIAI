"""
HGB + ExtraTrees 앙상블 — parquet v3 피처 검증
- 캐시된 hgb_v2 + extratrees_v2 params 로드
- parquet_features_v3 (심층 센서 피처) 적용
- 10 seeds 멀티 시드 앙상블, 동일 가중치 평균
- OOF 기준 HGB+ET v2(0.6434) 대비 성능 비교
출력: submission/hgb_et_v4_ensemble_prob.csv (clip 0.1~0.9)
"""
...
# v2 대비 비교 기준값
HGB_LL_V2 = {"Q1": 0.6988, "Q2": 0.6577, "Q3": 0.6336,
              "S1": 0.6141, "S2": 0.6193, "S3": 0.6230, "S4": 0.6795}
ET_LL_V2  = {"Q1": 0.6998, "Q2": 0.6478, "Q3": 0.6443,
              "S1": 0.6161, "S2": 0.6161, "S3": 0.6124, "S4": 0.6870}
ENS_LL_V2 = {"Q1": 0.6974, "Q2": 0.6509, "Q3": 0.6330,
...
def train_and_predict(train, test, parquet_feat):
    ...
    print(f"  ET  ({ET_KEY}):  Q1 n_est={cached_et['Q1']['n_estimators']}, "
          f"depth={cached_et['Q1']['max_depth']}\n")

    # ── Phase 2: 멀티 시드 앙상블 ─────────────────────────────────────────
    n_seeds = len(SEEDS)
    print(f"=== Phase 2: HGB+ET 앙상블 v4 ({n_seeds} seeds) ===")
    ...
    # ── 결과 출력 ────────────────────────────────────────────────────────────
    print()
    print(f"{'타깃':<5}  {'v4 F1':>6}  {'v4 LL':>7}  {'v2 LL':>7}  {'차이':>6}")
    print("-" * 42)
    for t in TARGETS:
        ...
        diff = ll - ENS_LL_V2[t]
        sign = "+" if diff > 0 else ""
        print(f"{t:<5}  {f1:>6.3f}  {ll:>7.4f}  {ENS_LL_V2[t]:>7.4f}  {sign}{diff:>5.4f}")

    avg_ll_v4 = np.mean([log_loss(train[t].values, ens_oof[t]) for t in TARGETS])
    avg_f1_v4 = np.mean([f1_score(train[t].values, (ens_oof[t] > 0.5).astype(int)) for t in TARGETS])
    avg_diff  = avg_ll_v4 - 0.6434
    sign = "+" if avg_diff > 0 else ""
    print(f"{'평균':<5}  {avg_f1_v4:>6.3f}  {avg_ll_v4:>7.4f}  {'0.6434':>7}  {sign}{avg_diff:>5.4f}")
...
def main():
    ...
    print("=== HGB + ExtraTrees 앙상블 v4 (parquet v3) ===\n")
