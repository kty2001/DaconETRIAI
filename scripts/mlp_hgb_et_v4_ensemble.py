"""
Multi-task MLP + HGB + ExtraTrees 3모델 앙상블 — parquet v3 피처
- MLP: mlp_v1 params + v3 피처 (subj_mean 전체 제거, 다양성 유지)
- HGB: hgb_v3 params (v3 피처 기준 Optuna 재튜닝)
- ET:  extratrees_v3 params (v3 피처 기준 Optuna 재튜닝)
- 동일 가중치 (1/3 씩)

기준선: MLP+HGB+ET v2 OOF 0.6383
출력: submission/mlp_hgb_et_v4_ensemble_prob.csv (clip 0.1~0.9)
"""
...
# v2 기준선 (비교용)
REF_ENS_LL = {"Q1": 0.6994, "Q2": 0.6478, "Q3": 0.6443,
               "S1": 0.6161, "S2": 0.6161, "S3": 0.6124, "S4": 0.6870}
REF_MLP_LL = {"Q1": 0.7145, "Q2": 0.6587, "Q3": 0.6514,
...
def precompute_fold_features(train, test, parquet_feat, le, fold_label_feats, label_feat_test):
    ...
    # ── MLP 피처 (subj_mean 전체 + mUsageStats 제거 + fold imputation + scaling) ─
    fold_mlp = []
    for tr_idx, val_idx, X_tr, X_val in fold_raw:
...
def train_and_predict(train, test, parquet_feat):
    ...
    tree_n = fold_by_target[TARGETS[0]][0][2].shape[1]
    print(f"  완료 (트리 피처: {tree_n}개, MLP 입력: {mlp_input_dim}개, device: {DEVICE})\n")
    ...
    if not cached_hgb:
        raise RuntimeError(f"캐시에 {HGB_KEY} params 없음. hgb_et_v4_optuna.py 먼저 실행하세요.")
    if not cached_et:
        raise RuntimeError(f"캐시에 {ET_KEY} params 없음. hgb_et_v4_optuna.py 먼저 실행하세요.")
    ...
    print(f"  HGB ({HGB_KEY}):  Q1 lr={cached_hgb['Q1']['learning_rate']:.4f}, "
          f"iter={cached_hgb['Q1']['max_iter']}")
    print(f"  ET  ({ET_KEY}):   Q1 n_est={cached_et['Q1']['n_estimators']}, "
          f"depth={cached_et['Q1']['max_depth']}, max_feat={cached_et['Q1']['max_features']:.3f}")
    print(f"  MLP ({MLP_KEY}):  hidden={mlp_params['hidden_dims']}, "
          f"drop={mlp_params['dropout']:.3f}, ep={mlp_params['n_epochs']}\n")
    ...
    print(f"=== Phase 2: MLP+HGB+ET 3모델 앙상블 v4 ({n_seeds} seeds) ===")
    ...
    for seed_i, seed in enumerate(SEEDS):
        ...
        f1s = [f1_score(train[t].values,
                        (ens_oof[t] * n_seeds / (seed_i + 1) > 0.5).astype(int))
               for t in TARGETS]
    ...
    # ── 최종 결과 ─────────────────────────────────────────────────────────────
    print()
    print(f"{'타깃':<5}  {'F1':>6}  {'앙상블 LL':>9}  {'v2 ens':>7}  {'MLP v2':>7}  {'개선':>7}")
    print("-" * 52)
    for t in TARGETS:
        f1   = f1_score(train[t].values, (ens_oof[t] > 0.5).astype(int))
        ll   = log_loss(train[t].values, ens_oof[t])
        diff = ll - REF_ENS_LL[t]
        print(f"{t:<5}  {f1:>6.3f}  {ll:>9.4f}  "
              f"{REF_ENS_LL[t]:>7.4f}  {REF_MLP_LL[t]:>7.4f}  {diff:>+7.4f}")
    ...
    avg_f1 = np.mean([f1_score(train[t].values, (ens_oof[t] > 0.5).astype(int))
                      for t in TARGETS])
    print(f"{'평균':<5}  {avg_f1:>6.3f}  {avg_ll:>9.4f}  "
          f"{'0.6434':>7}  {'0.6398':>7}  {avg_ll - 0.6383:>+7.4f}  (vs v2 앙상블 0.6383)")
...
def main():
    ...
    print("=== MLP + HGB + ET 3모델 앙상블 v4 (parquet v3) ===\n")
