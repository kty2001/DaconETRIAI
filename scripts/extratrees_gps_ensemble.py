"""
ExtraTrees + GPS 피처 앙상블
- parquet v2 피처 + gps_features (mGps.parquet) 추가
- Optuna 100 trials/target, 10 seeds 멀티 시드
- GPS 피처도 subject z-score 정규화 (sensor_cols에 포함)
출력: submission/extratrees_gps_prob.csv (clip 0.1~0.9)
"""
...
def train_and_predict(train, test, parquet_feat):
    ...
    if cached_et:
        ...
        for t in TARGETS:
            ...
            best_et[t] = bp
            v2_ll = ET_LL_V2.get(t, "-")
            print(f"  {t}: LL={study.best_value:.4f} (v2: {v2_ll}) | "
    ...
    # ── 최종 OOF 결과 ────────────────────────────────────────────────────────
    print()
    print(f"{'타깃':<5}  {'F1':>6}  {'OOF LL':>8}  {'v2 LL':>8}  {'개선':>7}")
    print("-" * 40)
    lls_final = []
    for t in TARGETS:
        ...
        diff = ET_LL_V2[t] - ll
        sign = "[+]" if diff > 0 else "[-]"
        print(f"{t:<5}  {f1:>6.3f}  {ll:>8.4f}  {ET_LL_V2[t]:>8.4f}  {diff:>+6.4f} {sign}")
    avg_ll = np.mean(lls_final)
    avg_v2 = np.mean(list(ET_LL_V2.values()))
    print(f"{'평균':<5}  {'':>6}  {avg_ll:>8.4f}  {avg_v2:>8.4f}  {avg_v2-avg_ll:>+6.4f}")
...
def main():
    ...
    gps_cols = [c for c in gps_feat.columns if c not in ("subject_id", "date")]
    nan_rate = parquet_feat[gps_cols].isna().mean()
    print(f"  GPS NaN 비율 (평균): {nan_rate.mean():.3f}")
    print()

    print(f"=== ExtraTrees + GPS 앙상블 ({N_TRIALS} trials) ===\n")
    ...
    out_path = SUBMISSION_DIR / "extratrees_gps_prob.csv"
