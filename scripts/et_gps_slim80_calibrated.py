"""
ET GPS Slim 80% + 피험자별 로짓 편향 보정 (Logit Bias Calibration)

기존 ET GPS Slim 80% 위에 피험자별 보정 레이어를 추가:
...
LOSO OOF 기반 보정의 특성:
  test 예측은 9/10 폴드가 해당 피험자의 훈련 데이터를 본 상태이므로
  OOF 기반 bias는 실제 필요량보다 클 수 있음 -> 정규화(REG)로 과보정 방지

출력: submission/et_gps_slim80_calibrated_prob.csv
"""
...
CALIB_REG  = 0.5   # 로짓 편향 L2 정규화 강도 (클수록 보정 약해짐)
BIAS_BOUND = 2.0   # 편향 탐색 범위 [-BIAS_BOUND, +BIAS_BOUND]
...
def fit_logit_bias(pred_oof, y_true, reg=CALIB_REG, bound=BIAS_BOUND):
    """
    LOSO OOF 예측 기반 로짓 편향 추정.
    min_b: log_loss(y, expit(logit(pred) + b)) + reg * b^2

    반환값:
        bias (float): 양수이면 예측 확률을 높이는 방향
    """
    eps = 1e-6
...
def main():
    ...
    out_path = SUBMISSION_DIR / "et_gps_slim80_calibrated_prob.csv"
