"""
LightGBM + 동일 타깃 lag 사용 허용
lgbm_roll14lag7.py 기반에서 동일 타깃의 lag/roll 제거 방침 제거
- lag1/2/7, roll3/7/14를 동일 타깃 예측에도 사용
  (기존: Q1 예측 시 lag1_Q1 등 제거 → 개선: 제거하지 않음)
- subj_mean_t만 제거 유지 (타깃별 subject 평균, 누수 방지)
출력: submission/lgbm_lag_full_prob.csv (clip 0.1~0.9)
"""
...
def train_and_predict(train: pd.DataFrame, test: pd.DataFrame, parquet_feat: pd.DataFrame) -> pd.DataFrame:
    ...
    for seed_i, seed in enumerate(SEEDS):
        ...
        for t in TARGETS:
            y        = train[t].values
            # subj_mean_t만 제거 (lag/roll은 동일 타깃도 허용)
...
def main():
    ...
    print(f"=== LightGBM 동일 타깃 lag 허용 + 멀티 시드 ({len(SEEDS)}개) ===\n")
    ...
    out_path = SUBMISSION_DIR / "lgbm_lag_full_prob.csv"
