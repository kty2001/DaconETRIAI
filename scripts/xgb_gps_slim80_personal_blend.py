"""
XGB GPS Slim 80% Personal Blend (WS OOF Optuna)

Global: XGBoost Classifier (depth-based 트리, WS OOF Optuna 30 trials/target)
Personal: ExtraTrees depth=2

XGBoost 특징:
  - depth-based 트리: LGB(leaf-wise), CB(symmetric), ET(random)와 다른 귀납적 편향
  - L1/L2 동시 정규화 지원
  - 이전 ET(0.5989) + LGB(0.6079) + CB(WS OOF 0.6485) 앙상블에 다양성 추가

출력:
  submission/xgb_gps_slim80_personal_blend_prob.csv
"""
...
def main():
    ...
    fimp_params = load_params(FIMP_KEY)
    if not fimp_params:
    ...
    for tr_idx, val_idx in fold_indices:
        ...
        for t in TARGETS:
            ...
            if len(np.unique(y_tr)) < 2:
                ...
                continue
            model = ExtraTreesClassifier(**{**fimp_params[t], "random_state": WS_SEED})
    ...
    # ── Phase 1: Full data + test feature matrices ──
    print("\n=== Phase 1: Full data / test feature matrices ===", flush=True)
    ...
    for tt in TARGETS:
        drop_cols = [f"subj_mean_{tt}"] + DROP_USAGE
        first_sid = subjects[0]
        cols_tt = [c for c in keep_feats
                   if c in ws_cache[first_sid]["X_tr"].columns and c not in drop_cols]
        ws_cols[tt] = cols_tt
    ...
    # ── Phase 3: Optuna (XGB WS OOF) ──
    partial_cached = load_params(PARAMS_KEY) or {}
    missing = [t for t in TARGETS if t not in partial_cached]

    if not missing:
        print(f"\n=== Phase 3: Optuna 캐시 로드 ({PARAMS_KEY}) ===", flush=True)
        xgb_params = partial_cached
    else:
        xgb_params = dict(partial_cached)
        print(f"\n=== Phase 3: Optuna XGB WS OOF ({N_TRIALS} trials/target) ===", flush=True)
        ...
        for t in missing:
            ...
            def objective(trial, _t=t, _cols=cols_t):
                ...
                for sid in subjects:
                    ...
                    model = XGBClassifier(**{**XGB_FIXED, **params, "random_state": WS_SEED})
            ...
            study.optimize(objective, n_trials=N_TRIALS, timeout=400, show_progress_bar=False)
            xgb_params[t] = study.best_params
            print(f"  {t}: best LL={study.best_value:.4f}  {study.best_params}", flush=True)
            save_params(PARAMS_KEY, xgb_params)

    # ── Phase 4: WS OOF (Global XGB + Personal ET) ──
    print("\n=== Phase 4: WS OOF (Global XGB + Personal ET) ===", flush=True)
    ...
    for sid in subjects:
        ...
        for t in TARGETS:
            ...
            if len(np.unique(y_tr)) >= 2:
                glb = XGBClassifier(**{**XGB_FIXED, **xgb_params[t], "random_state": WS_SEED})
                glb.fit(X_tr_f, y_tr)
    ...
    # ── Phase 5: WS OOF LL + Alpha ──
    print("\n=== Phase 5: WS OOF LL ===", flush=True)
    for t in TARGETS:
    ...
    print("\n=== Phase 5b: Alpha optimization ===", flush=True)
    ...
    best_g  = alphas[int(np.argmin(grid_ll))]
    res = minimize_scalar(blend_logloss,
                          bounds=(max(0.0, best_g - 0.05), min(1.0, best_g + 0.05)),
                          method="bounded")
    best_alpha = float(res.x)
    best_ll    = float(res.fun)
    print(f"  alpha={best_alpha:.4f}, WS OOF blend LL={best_ll:.4f}", flush=True)

    # ── Phase 6: Global XGB (10 seeds) + Personal ET (10 seeds) ──
    print(f"\n=== Phase 6: Global XGB model ({len(SEEDS)} seeds) ===", flush=True)
    xgb_test_global = {t: np.zeros(len(sample)) for t in TARGETS}
    for seed in SEEDS:
        for t in TARGETS:
            cols_t  = ws_cols[t]
            ...
            model = XGBClassifier(**{**XGB_FIXED, **xgb_params[t], "random_state": seed})
            model.fit(X_all_t.fillna(all_med), train_reset[t].values)
            xgb_test_global[t] += model.predict_proba(X_te_t.fillna(all_med))[:, 1] / len(SEEDS)
        print(f"  seed {seed} done", flush=True)

    print(f"\n=== Phase 6b: Personal ET ({len(SEEDS)} seeds) ===", flush=True)
    xgb_test_pers = {t: np.zeros(len(sample)) for t in TARGETS}
    for sid in subjects:
        ...
        for t in TARGETS:
            ...
            xgb_test_pers[t][mask_te] = preds_sid
        print(f"  {sid}: {n_pers} days done", flush=True)

    # ── Phase 7: Blend + Logit bias correction ──
    print(f"\n=== Phase 7: Blend (alpha={best_alpha:.4f}) + bias correction ===", flush=True)
    xgb_test_blend = {t: blend_probs(xgb_test_global[t], xgb_test_pers[t], best_alpha)
                      for t in TARGETS}
    ...
    xgb_test_cal = {t: xgb_test_blend[t].copy() for t in TARGETS}
    for sid in subjects:
        mask_te = (sample["subject_id"] == sid).values
        if mask_te.sum() == 0:
            continue
        for t in TARGETS:
            xgb_test_cal[t][mask_te] = apply_logit_bias(
                xgb_test_blend[t][mask_te], biases[sid][t]
            )

    result_prob = sample[["subject_id", "sleep_date", "lifelog_date"]].copy()
    for t in TARGETS:
        result_prob[t] = xgb_test_cal[t].clip(0.1, 0.9)
    ...
    out_path = SUBMISSION_DIR / "xgb_gps_slim80_personal_blend_prob.csv"
