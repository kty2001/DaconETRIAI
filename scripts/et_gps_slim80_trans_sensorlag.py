"""
ET GPS Slim 80% + Transductive Z-score + 센서 Lag 피처 + 피험자별 로짓 편향 보정

et_gps_slim80_transductive.py 대비 추가 사항:
  - 센서 lag 피처: sensorlag1_{col} (전날 센서), sensorroll7_{col} (직전 7 레코딩 평균)
  - parquet_feat 전체 날짜 (train + test) 기반 -> 정보 유출 없음

파이프라인:
  - Phase 0: Feature Importance (extratrees_gps 캐시 활용)
  - Phase 1: Optuna 파라미터 로드 (extratrees_gps_slim80 캐시 활용)
  - Phase 2: 10 seeds 앙상블 & 피험자별 OOF 수집
  - Phase 3: 피험자 x 타깃별 로짓 편향 fitting (REG=0.5)
  - Phase 4: 보정된 test 예측 저장

출력: submission/et_gps_slim80_trans_sensorlag_prob.csv
"""
...
from sensor_lag_features import build_sensor_lags
...
def build_features(df, ref, parquet_feat, label_feat, sensor_lag_feat, is_train, le):
    ...
    df = df.merge(label_feat, left_on=["subject_id", "lifelog_date"],
                  right_on=["subject_id", "date"], how="left").drop(columns=["date"], errors="ignore")
    df = df.merge(sensor_lag_feat, left_on=["subject_id", "lifelog_date"],
                  right_on=["subject_id", "date"], how="left").drop(columns=["date"], errors="ignore")
    ...
    sensor_lag_cols = [c for c in sensor_lag_feat.columns if c not in ("subject_id", "date")]
    all_cols = ["subject_id"] + base_cols + parquet_cols + label_cols + sensor_lag_cols
...
def compute_importance(train, parquet_feat, le, sensor_cols, fold_label_feats,
                       best_gps, transductive_stats, sensor_lag_feat):
    importance_dict = {t: [] for t in TARGETS}
    feature_names_ref = {}

    for tr_idx, val_idx, train_fold, val_fold, lf_tr, lf_val in fold_label_feats:
        X_tr   = build_features(train_fold, train_fold, parquet_feat,
                                lf_tr, sensor_lag_feat, True, le)
...
def precompute_fold_features(train, test, parquet_feat, le, sensor_cols,
                             ...
                             transductive_stats, sensor_lag_feat, keep_feats):
    fold_base = []
    for tr_idx, val_idx, train_fold, val_fold, lf_tr, lf_val in fold_label_feats:
        X_tr  = build_features(train_fold, train_fold, parquet_feat,
                               lf_tr,  sensor_lag_feat, True,  le)
        X_val = build_features(val_fold,   train_fold, parquet_feat,
                               lf_val, sensor_lag_feat, False, le)
        sid_tr  = X_tr["subject_id"].reset_index(drop=True)
    ...
    X_te_raw = build_features(test.copy(), train, parquet_feat,
                              label_feat_test, sensor_lag_feat, False, le)
...
def train_and_predict(train, test, parquet_feat, sensor_lag_feat):
    ...
    print(f"  sensor lag 피처: {len([c for c in sensor_lag_feat.columns if c not in ('subject_id','date')])}개 컬럼")
    ...
    keep_feats = compute_importance(train, parquet_feat, le, sensor_cols,
                                    ...
                                    transductive_stats, sensor_lag_feat)
    print()

    print("폴드 피처 사전 계산 중 (slim 80% + transductive + sensor lag)...")
    ...
    if slim_params:
        ...
        print(f"ERROR: {ET_KEY_SLIM} params 캐시 없음.")
        return None
    ...
    # Phase 3: 피험자별 로짓 편향 보정
    print(f"\n=== Phase 3: 피험자별 로짓 편향 보정 (REG={CALIB_REG}) ===")
    ...
    print("      Public score로 실제 효과 확인 필요.")
...
def main():
    ...
    print("=== 센서 lag 피처 계산 중 ===")
    sensor_lag_feat = build_sensor_lags(parquet_feat)
    lag1_cols = [c for c in sensor_lag_feat.columns if c.startswith("sensorlag1_")]
    roll7_cols = [c for c in sensor_lag_feat.columns if c.startswith("sensorroll7_")]
    print(f"  sensorlag1: {len(lag1_cols)}개, sensorroll7: {len(roll7_cols)}개")
    print()

    print("=== ET GPS Slim 80% + Transductive Z-score + 센서 Lag + 피험자별 보정 ===\n")
    result = train_and_predict(train, sample, parquet_feat, sensor_lag_feat)
    if result is None:
        return
    ...
    out_path = SUBMISSION_DIR / "et_gps_slim80_trans_sensorlag_prob.csv"
