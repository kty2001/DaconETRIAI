"""
LightGBM + Platt Scaling Calibration
lgbm_multiseed.py와 동일한 LOSO 학습 후, OOF 예측으로 타깃별 보정 파라미터 학습
- 기존: raw 확률에 고정 clip(0.1~0.9) 적용
- 개선: OOF 예측으로 LogisticRegression(1D) 피팅 → 타깃별 최적 sigmoid 변환
출력: submission/lgbm_calibrated_prob.csv (clip 0.001~0.999)
"""
...
def train_and_predict(train: pd.DataFrame, test: pd.DataFrame, parquet_feat: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── OOF 최종 성능 ─────────────────────────────────────────────────────────
    print()
    print(f"{'타깃':<5}  {'OOF F1':>8}  {'OOF LogLoss':>12}  {'기준 F1':>8}  {'차이':>6}")
    print("-" * 52)
    for t in TARGETS:
    ...
    print("\n=== Platt Scaling Calibration ===")
    print(f"{'타깃':<5}  {'Raw LL':>8}  {'Cal LL':>8}  {'개선':>8}")
    print("-" * 38)
    ...
    for t in TARGETS:
        ...
        oof_pred = oof_accum[t]
        te_pred  = test_accum[t]

        raw_ll = log_loss(y_true, oof_pred)

        # Platt Scaling: LogisticRegression on 1D OOF predictions
        cal = LogisticRegression(C=1.0, solver="lbfgs", max_iter=1000)
        cal.fit(oof_pred.reshape(-1, 1), y_true)
        cal_oof  = cal.predict_proba(oof_pred.reshape(-1, 1))[:, 1]
        cal_test = cal.predict_proba(te_pred.reshape(-1, 1))[:, 1]

        cal_ll = log_loss(y_true, cal_oof)
        diff   = cal_ll - raw_ll
        sign   = "+" if diff >= 0 else ""
        print(f"{t:<5}  {raw_ll:>8.4f}  {cal_ll:>8.4f}  {sign}{diff:>7.4f}")
...
def main():
    ...
    print(f"=== LightGBM 멀티 시드 ({len(SEEDS)}개) + Platt Scaling Calibration ===\n")
    result = train_and_predict(train, sample, parquet_feat)

    result_prob = result[["subject_id", "sleep_date", "lifelog_date"]].copy()
    for t in TARGETS:
        result_prob[t] = result[t].clip(0.001, 0.999)

    print("\n=== 예측 분포 (clip 0.001~0.999) ===")
    for t in TARGETS:
        print(f"  {t}: min={result_prob[t].min():.4f}, "
              f"mean={result_prob[t].mean():.4f}, "
              f"max={result_prob[t].max():.4f}")

    out_path = SUBMISSION_DIR / "lgbm_calibrated_prob.csv"
