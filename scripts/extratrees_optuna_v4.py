"""
ExtraTrees Optuna v4: max_features 0.1~0.3 집중 재탐색
- parquet v2 피처 + subject z-score (ET v2 최고 조합 유지)
- max_features 탐색 범위 0.1~0.3 집중 (v2 최적 대부분 이 범위에 집중)
- n_estimators 200~1500 (step 50), max_depth 5~25
- Optuna 200 trials/target
- 10 seeds 멀티 시드 앙상블, clip [0.1, 0.9]
출력: submission/extratrees_optuna_v4_prob.csv
"""
...
# max_features 0.1~0.3 집중 탐색 (v2 최적 대부분 이 범위)
ET_SEARCH = {
    "n_estimators":      ("int",   200,  1500, {"step": 50}),
    "max_depth":         ("int",   5,    25),
    "min_samples_split": ("int",   2,    10),
    "min_samples_leaf":  ("int",   1,    5),
    "max_features":      ("float", 0.1,  0.3),
}
ET_FIXED = {
    "criterion": "entropy",
    "n_jobs":    -1,
...
def train_and_predict(train, test, parquet_feat):
    ...
    if cached_et:
        ...
        print(f"=== Phase 1: ET Optuna v4 ({N_TRIALS} trials/target) ===")
        print(f"  탐색 공간: max_features=[0.1, 0.3], n_est=[200, 1500 step50], depth=[5, 25]\n")
        best_et = {}
        for t in TARGETS:
            y = train[t].values
            ...
            best_et[t] = bp
            v2_ll = ET_LL_V2[t]
            diff  = study.best_value - v2_ll
            sign  = "+" if diff > 0 else ""
            print(f"  {t}: LL={study.best_value:.4f} (v2={v2_ll:.4f}, {sign}{diff:.4f}) | "
...
def main():
    ...
    print("=== parquet v2 피처 집계 중 ===")
    parquet_feat = build_parquet_features()
    print()

    print(f"=== ET Optuna v4 앙상블 (max_feat 0.1~0.3, {N_TRIALS} trials) ===\n")
    ...
    out_path = SUBMISSION_DIR / "extratrees_optuna_v4_prob.csv"
