"""
ExtraTrees v4 앙상블: parquet v4 피처 + 윈도우 페어 trend 피처 + anchor_stage1 파라미터
- parquet_features_v4 (123컬럼: v3 117 + hr_extsleep 4 + mACStatus 4)
- label_features.py: trend_short/l1r7/long 21개 포함 (총 93열)
- anchor_stage1 Optuna params 재사용 (100 trials, 이미 캐시됨)
- 10 seeds, clip [0.1, 0.9]
출력: submission/extratrees_v4_ensemble_prob.csv
"""
...
ET_KEY   = "anchor_stage1"  # v4 피처 기준 Optuna params (이미 캐시됨)
N_TRIALS = 100
...
def train_and_predict(train, test, parquet_feat):
    ...
    print(f"=== Phase 2: ET v4 앙상블 ({n_seeds} seeds) ===")
    ...
    for seed_i, seed in enumerate(SEEDS):
        ...
        for t in TARGETS:
            ...
            for tr_idx, val_idx, X_tr, X_val in fold_by_target[t]:
                ...
                seed_test[t]         += et.predict_proba(te_by_target[t].fillna(tr_median))[:, 1] / n_folds
...
def main():
    ...
    print(f"=== ET v4 앙상블 (parquet_v4 + trend 피처, {ET_KEY} params) ===\n")
    ...
    out_path = SUBMISSION_DIR / "extratrees_v4_ensemble_prob.csv"
