...
lgbm_lag_full.py 기반 (동일 타깃 lag 허용)
- Phase 1: 타깃별 Optuna (n_trials=30) → best_params 탐색
  피처를 폴드별로 1회 사전 계산해 trial 속도 최대화
- Phase 2: best_params + 10 seeds로 최종 예측
출력: submission/lgbm_optuna_prob.csv (clip 0.1~0.9)
"""
...
def main():
    ...
    print(f"=== LightGBM + Optuna ({N_TRIALS} trials/target) + 멀티 시드 ({len(SEEDS)}개) ===\n")
    ...
    out_path = SUBMISSION_DIR / "lgbm_optuna100_prob.csv"
