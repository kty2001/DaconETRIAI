...
CB_SEARCH = {
    "iterations":        ("int",   200,  2000, {"step": 50}),
...
def make_cb_objective(search_space, fixed_params, fold_features_t, y):
    def objective(trial):
        params = {**fixed_params}
...
def train_and_predict(train, test, parquet_feat):
    ...
    cached_cb = load_params(CB_KEY)
    if cached_cb:
        print(f"=== Phase 1: 저장된 params 로드 ({CB_KEY}) ===")
        best_cb = {t: cached_cb[t] for t in TARGETS}
        for t in TARGETS:
            bp = best_cb[t]
            print(f"  {t}: iter={bp.get('iterations')}, lr={bp.get('learning_rate'):.4f}, "
                  f"depth={bp.get('depth')}")
        print()
    else:
        print(f"=== Phase 1: CatBoost Optuna slim85 ({N_TRIALS} trials/target) ===")
        best_cb = {}
        for t in TARGETS:
            y = train[t].values
            study = optuna.create_study(direction="minimize",
                                        sampler=optuna.samplers.TPESampler(seed=42))
            study.optimize(make_cb_objective(CB_SEARCH, CB_FIXED, fold_by_target[t], y),
                           n_trials=N_TRIALS, show_progress_bar=True)
            bp = {**CB_FIXED, **study.best_params}
            best_cb[t] = bp
            print(f"  {t}: LL={study.best_value:.4f} | "
                  f"iter={bp['iterations']}, lr={bp['learning_rate']:.4f}, "
                  f"depth={bp['depth']}")
        print()
        save_params(CB_KEY, best_cb)

    print(f"=== Phase 2: CatBoost slim85 앙상블 ({n_seeds} seeds) ===")
    print(f"{'시드':>6}  " + "  ".join(f"{t:>6}" for t in TARGETS) + "  평균F1   평균LL")
    print("-" * (8 + 9 * len(TARGETS) + 16))

    cb_oof  = {t: np.zeros(len(train)) for t in TARGETS}
    cb_test = {t: np.zeros(len(test))  for t in TARGETS}

    for seed_i, seed in enumerate(SEEDS):
        seed_cb_oof  = {t: np.zeros(len(train)) for t in TARGETS}
        seed_cb_test = {t: np.zeros(len(test))  for t in TARGETS}

        for t in TARGETS:
            y = train[t].values
            params = {**best_cb[t], "random_seed": seed}
            for tr_idx, val_idx, X_tr, X_val in fold_by_target[t]:
                ...
                model.fit(X_tr_filled, y[tr_idx])
                seed_cb_oof[t][val_idx] += model.predict_proba(X_val_filled)[:, 1]
                seed_cb_test[t]         += model.predict_proba(X_te_filled)[:, 1] / cv.n_splits

        for t in TARGETS:
            cb_oof[t]  += seed_cb_oof[t]  / n_seeds
            cb_test[t] += seed_cb_test[t] / n_seeds

        f1s, lls = [], []
        for t in TARGETS:
            cur = cb_oof[t] * n_seeds / (seed_i + 1)
    ...
    lls_final = []
    for t in TARGETS:
        f1 = f1_score(train[t].values, (cb_oof[t] > 0.5).astype(int))
        ll = log_loss(train[t].values, cb_oof[t])
        lls_final.append(ll)
...
def main():
    ...
    print(f"=== CatBoost GPS Slim {IMP_COVERAGE*100:.0f}% ===\n")
    ...
    out_path = SUBMISSION_DIR / "catboost_gps_slim85_prob.csv"
