"""
ExtraTrees + GPS Slim 앙상블
- parquet v2 + GPS 피처에서 Feature Importance 하위 피처 제거 (상위 90% 커버)
- Phase 0: 중요도 계산 (extratrees_gps params 활용)
- Phase 1: Optuna 100 trials (새 키 extratrees_gps_slim)
- Phase 2: 10 seeds 앙상블
출력: submission/extratrees_gps_slim_prob.csv (clip 0.1~0.9)
"""
...
ET_KEY_SLIM = "extratrees_gps_slim"
N_TRIALS    = 100
IMP_COVERAGE = 0.90   # 상위 90% 중요도 커버 피처만 유지
...
def compute_importance(train, parquet_feat, le, sensor_cols, fold_label_feats, best_et):
    """GPS params로 Feature Importance 계산 (seed=42)"""
    ...
    print(f"  상위 {IMP_COVERAGE*100:.0f}% 중요도 커버: {len(keep)}개 피처 유지")
    print(f"  제거: {len(combined)-len(keep)}개 피처")
    return set(keep)
...
def precompute_fold_features(train, test, parquet_feat, le, sensor_cols,
    ...
    for t in TARGETS:
        ...
        # 첫 번째 폴드 기준으로 기준 컬럼 순서 확정 (이후 모든 폴드/테스트에 동일 적용)
...
def train_and_predict(train, test, parquet_feat):
    ...
    if cached_et:
        ...
        print(f"=== Phase 1: ExtraTrees Optuna slim ({N_TRIALS} trials/target) ===")
        best_et = {}
        for t in TARGETS:
            y = train[t].values
            ...
            best_et[t] = bp
            gps_ll = ET_LL_GPS.get(t, "-")
            print(f"  {t}: LL={study.best_value:.4f} (GPS: {gps_ll}) | "
    ...
    # Phase 2: 멀티 시드 예측
    print(f"=== Phase 2: ExtraTrees slim 앙상블 ({n_seeds} seeds) ===")
    ...
    print()
    print(f"{'타깃':<5}  {'F1':>6}  {'OOF LL':>8}  {'GPS LL':>8}  {'개선':>7}")
    print("-" * 42)
    lls_final = []
    for t in TARGETS:
        ...
        diff = ET_LL_GPS[t] - ll
        sign = "[+]" if diff > 0 else "[-]"
        print(f"{t:<5}  {f1:>6.3f}  {ll:>8.4f}  {ET_LL_GPS[t]:>8.4f}  {diff:>+6.4f} {sign}")
    avg_ll  = np.mean(lls_final)
    avg_gps = np.mean(list(ET_LL_GPS.values()))
    print(f"{'평균':<5}  {'':>6}  {avg_ll:>8.4f}  {avg_gps:>8.4f}  {avg_gps-avg_ll:>+6.4f}")
...
def main():
    ...
    print(f"=== ExtraTrees GPS Slim ({IMP_COVERAGE*100:.0f}% importance coverage) ===\n")
    ...
    out_path = SUBMISSION_DIR / "extratrees_gps_slim_prob.csv"
