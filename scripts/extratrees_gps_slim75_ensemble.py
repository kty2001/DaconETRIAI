"""
ExtraTrees + GPS Slim 75% 앙상블
- parquet v2 + GPS 피처, Feature Importance 상위 85% 커버 피처만 유지
- Phase 0: 중요도 계산 (extratrees_gps params 활용)
- Phase 1: Optuna 100 trials (extratrees_gps_slim75)
- Phase 2: 10 seeds 앙상블
출력: submission/extratrees_gps_slim75_prob.csv (clip 0.1~0.9)
"""
...
ET_KEY_SLIM = "extratrees_gps_slim75"
N_TRIALS    = 100
IMP_COVERAGE = 0.75
...
def train_and_predict(train, test, parquet_feat):
    ...
    if cached_et:
        ...
        print(f"=== Phase 1: ExtraTrees Optuna slim75 ({N_TRIALS} trials/target) ===")
        best_et = {}
        for t in TARGETS:
            y = train[t].values
    ...
    print(f"=== Phase 2: ExtraTrees slim75 앙상블 ({n_seeds} seeds) ===")
...
def main():
    ...
    print(f"=== ExtraTrees GPS Slim 75% ===\n")
    ...
    out_path = SUBMISSION_DIR / "extratrees_gps_slim75_prob.csv"
