"""
ET GPS Slim 80% + Transductive Z-score + 피험자별 로짓 편향 보정

기존 ET GPS Slim 80% + 로짓 편향 보정 위에 Transductive Z-score 정규화 추가:
  ...
  - Phase 3: 피험자 x 타깃별 로짓 편향 fitting
             logit(p_cal) = logit(p_raw) + bias
             min: log_loss(y, expit(logit(p_oof) + bias)) + REG * bias^2
  - Phase 4: 보정된 test 예측 저장

Transductive Z-score (vs et_gps_slim80_calibrated.py):
  train 폴드만으로 피험자별 평균/표준편차를 계산하는 대신,
  parquet_feat (train + test 날짜 모두 포함) 에서 피험자별 통계를 계산.
  레이블 미사용 (X 만 활용) -> 정보 유출 없음.
  훈련 날짜가 적은 피험자 (id03, id10: 33 일) 의 기저선 추정 정확도 향상 기대.

출력: submission/et_gps_slim80_transductive_prob.csv
"""
...
def compute_transductive_stats(parquet_feat, sensor_cols):
    """
    parquet_feat (train + test 날짜 모두 포함) 에서 피험자별 센서 통계 계산.
    레이블을 사용하지 않으므로 정보 유출 없음.
    """
...
def train_and_predict(train, test, parquet_feat):
    ...
    # Transductive stats: train + test 날짜 모두 포함한 parquet_feat 에서 계산
...
def main():
    ...
    print("=== ET GPS Slim 80% + Transductive Z-score + 피험자별 보정 ===\n")
    ...
    out_path = SUBMISSION_DIR / "et_gps_slim80_transductive_prob.csv"
