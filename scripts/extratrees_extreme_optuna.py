...
- max_features 탐색 범위 0.05~0.15 집중 (v2 최적 0.11 주변, log scale)
- n_estimators 200~1500, max_depth 5~30으로 확대
- Optuna 300 trials/target
- 10 seeds 멀티 시드 앙상블
출력: submission/extratrees_extreme_prob.csv (clip 0.1~0.9)
"""
...
ET_KEY   = "extratrees_extreme"
N_TRIALS = 300

TARGETS = ["Q1", "Q2", "Q3", "S1", "S2", "S3", "S4"]
SEEDS   = [42, 123, 456, 789, 1024, 2024, 3141, 5678, 9999, 31415]

# ET v2 OOF LL 비교 기준 (Public 0.6088)
ET_LL_V2 = {
...
# max_features 0.05~0.15 집중 (log scale), n_estimators/depth 확대
ET_SEARCH = {
    "n_estimators":      ("int",   200,  1500, {"step": 50}),
    "max_depth":         ("int",   5,    30),
    "min_samples_split": ("int",   2,    10),
    "min_samples_leaf":  ("int",   1,    5),
    "max_features":      ("float", 0.05, 0.15, {"log": True}),
}
ET_FIXED = {
    "criterion": "entropy",
    "n_jobs":    -1,
...
def train_and_predict(train, test, parquet_feat):
    ...
    if cached_et:
        ...
        print(f"=== Phase 1: ExtraTrees 극한 Optuna ({N_TRIALS} trials/target) ===")
        print(f"  탐색 공간: max_features=[0.05, 0.15 log], n_est=[200, 1500], depth=[5, 30]\n")
        best_et = {}
        for t in TARGETS:
            y = train[t].values
    ...
    # ── Phase 2: 멀티 시드 앙상블 ─────────────────────────────────────────────
    print(f"=== Phase 2: ExtraTrees 극한 앙상블 ({n_seeds} seeds) ===")
    ...
    print()
    print(f"{'타깃':<5}  {'극한 F1':>7}  {'극한 LL':>8}  {'v2 LL':>7}  {'차이':>6}")
    print("-" * 44)
    for t in TARGETS:
        ...
        diff = ll - ET_LL_V2[t]
        sign = "+" if diff > 0 else ""
        print(f"{t:<5}  {f1:>7.3f}  {ll:>8.4f}  {ET_LL_V2[t]:>7.4f}  {sign}{diff:>5.4f}")
    ...
    diff   = avg_ll - 0.6462
    sign   = "+" if diff > 0 else ""
    print(f"{'평균':<5}  {avg_f1:>7.3f}  {avg_ll:>8.4f}  {'0.6462':>7}  {sign}{diff:>5.4f}")
...
def main():
    ...
    print(f"=== ExtraTrees 극한 최적화 ({N_TRIALS} trials/target) ===\n")
    ...
    out_path = SUBMISSION_DIR / "extratrees_extreme_prob.csv"
