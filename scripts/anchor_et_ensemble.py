"""
Anchor ET Ensemble: Stage 1 OOF를 앵커 피처로 활용한 2단계 ET 앙상블
- parquet v4 피처 + 윈도우 페어 label 피처 (trend_short/l1r7/long)
- Stage 1: Optuna 100 trials/target (ET_KEY='anchor_stage1')
           1 seed(42)로 LOSO OOF + test 예측 수집
- Stage 2: Stage 1 OOF 7컬럼을 앵커 피처로 추가, 10 seeds ET 앙상블
- clip [0.05, 0.95]
출력: submission/anchor_et_prob.csv
"""
...
from parquet_features_v4 import build_all as build_parquet_features
...
def make_et_objective(search_space, fixed_params, fold_features_t, y):
    def objective(trial):
        params = {**fixed_params, "random_state": 42}
        ...
        for tr_idx, val_idx, X_tr, X_val in fold_features_t:
            ...
            model.fit(X_tr.fillna(tr_median), y[tr_idx])
            oof[val_idx] = model.predict_proba(X_val.fillna(tr_median))[:, 1]
...
def train_and_predict(train, test, parquet_feat):
    ...
    # Phase 1: Optuna
    cached_s1 = load_params(ET_KEY_S1)
    if cached_s1:
        print(f"=== Phase 1: 저장된 params 로드 ({ET_KEY_S1}) ===")
        best_et = cached_s1
        for t in TARGETS:
            bp = best_et[t]
        ...
        print(f"=== Phase 1: Optuna ({N_TRIALS} trials/target) ===")
        best_et = {}
        for t in TARGETS:
            y = train[t].values
            ...
            study.optimize(
                make_et_objective(ET_SEARCH, ET_FIXED, fold_by_target[t], y),
                n_trials=N_TRIALS,
                show_progress_bar=False,
            )
            bp = {**ET_FIXED, **study.best_params, "random_state": 42}
            best_et[t] = bp
            diff = study.best_value - ET_LL_V2[t]
            sign = "+" if diff > 0 else ""
            print(f"  {t}: LL={study.best_value:.4f} (v2={ET_LL_V2[t]:.4f}, {sign}{diff:.4f}) | "
                  f"n_est={bp['n_estimators']}, depth={bp['max_depth']}, "
                  f"max_feat={bp['max_features']:.4f}")
        print()
        save_params(ET_KEY_S1, best_et)

    # Stage 1: LOSO OOF + test 수집 (seed=42)
    print("=== Stage 1: OOF 수집 (seed=42) ===")
    s1_oof  = {t: np.zeros(len(train)) for t in TARGETS}
    s1_test = {t: np.zeros(len(test))  for t in TARGETS}

    for t in TARGETS:
        y      = train[t].values
        params = {**best_et[t], "random_state": 42}
        for tr_idx, val_idx, X_tr, X_val in fold_by_target[t]:
            tr_median = X_tr.median()
            m = ExtraTreesClassifier(**params)
            m.fit(X_tr.fillna(tr_median), y[tr_idx])
            s1_oof[t][val_idx] = m.predict_proba(X_val.fillna(tr_median))[:, 1]
            s1_test[t] += m.predict_proba(te_by_target[t].fillna(tr_median))[:, 1] / n_folds

    s1_ll = np.mean([log_loss(train[t].values, s1_oof[t]) for t in TARGETS])
    print(f"  Stage 1 OOF LL: {s1_ll:.4f}")

    s1_oof_df  = pd.DataFrame({f"s1_{t}": s1_oof[t]  for t in TARGETS})
    s1_test_df = pd.DataFrame({f"s1_{t}": s1_test[t] for t in TARGETS})
    ...
    te_by_target_s2   = {}
    for tgt in TARGETS:
        fold_by_target_s2[tgt] = []
        for tr_idx, val_idx, X_tr, X_val in fold_by_target[tgt]:
            X_tr_aug  = pd.concat(
                [X_tr.reset_index(drop=True),  s1_oof_df.iloc[tr_idx].reset_index(drop=True)],  axis=1
            )
            X_val_aug = pd.concat(
                [X_val.reset_index(drop=True), s1_oof_df.iloc[val_idx].reset_index(drop=True)], axis=1
            )
            fold_by_target_s2[tgt].append((tr_idx, val_idx, X_tr_aug, X_val_aug))
        te_by_target_s2[tgt] = pd.concat(
            [te_by_target[tgt].reset_index(drop=True), s1_test_df.reset_index(drop=True)], axis=1
        )
    n_feats_s2 = fold_by_target_s2[TARGETS[0]][0][2].shape[1]
    print(f"  Stage 2 피처 수: {n_feats_s2}개 (Stage 1: {n_feats}개 + 앵커 7개)\n")

    # Stage 2: 10 seeds 앙상블
    print(f"=== Stage 2: 앙상블 ({n_seeds} seeds) ===")
    ...
    for seed_i, seed in enumerate(SEEDS):
        ...
        for t in TARGETS:
            ...
            for tr_idx, val_idx, X_tr, X_val in fold_by_target_s2[t]:
                tr_median = X_tr.median()
                et = ExtraTreesClassifier(**et_params)
                et.fit(X_tr.fillna(tr_median), y[tr_idx])
                seed_oof[t][val_idx]  = et.predict_proba(X_val.fillna(tr_median))[:, 1]
                seed_test[t]         += et.predict_proba(te_by_target_s2[t].fillna(tr_median))[:, 1] / n_folds
    ...
    print()
    print(f"{'타깃':<5}  {'F1':>6}  {'OOF LL':>8}  {'v2 LL':>7}  {'차이':>6}")
    print("-" * 44)
    for t in TARGETS:
        ...
        diff = ll - ET_LL_V2[t]
        sign = "+" if diff > 0 else ""
        print(f"{t:<5}  {f1:>6.3f}  {ll:>8.4f}  {ET_LL_V2[t]:>7.4f}  {sign}{diff:>5.4f}")
    ...
    diff   = avg_ll - 0.6462
    sign   = "+" if diff > 0 else ""
    print(f"{'평균':<5}  {avg_f1:>6.3f}  {avg_ll:>8.4f}  {'0.6462':>7}  {sign}{diff:>5.4f}")
...
def main():
    ...
    print("=== parquet v4 피처 집계 중 ===")
    parquet_feat = build_parquet_features()
    print()

    print(f"=== Anchor ET Ensemble (v4 피처 + 앵커 OOF, {N_TRIALS} trials) ===\n")
    ...
    out_path = SUBMISSION_DIR / "anchor_et_prob.csv"
