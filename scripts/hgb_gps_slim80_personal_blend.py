"""
HGB GPS Slim 80% Personal Blend

Global: HistGradientBoostingClassifier (WS OOF Optuna 30 trials/target)
Personal: ExtraTrees depth=2 (pers_grid_best 최적)
Feature: parquet v2 + GPS + label features, LOSO HGB 중요도 상위 80%
HGB는 NaN을 네이티브 처리하므로 global 모델에 fillna 불필요.

출력:
  submission/optuna_params.json -> hgb_gps_slim80_ws 키
  submission/hgb_gps_slim80_personal_blend_prob.csv
"""
...
def main():
    ...
    # ── Phase 0: HGB Feature Importance (LOSO 1 seed) ──
    print("\n=== Phase 0: HGB Feature Importance (LOSO 1 seed) ===", flush=True)
    ...
    print("\n=== Phase 4: WS OOF comparison (HGB-optuna) ===", flush=True)
    for t in TARGETS:
        cols_t = ws_cols[t]
    ...
    out_path = SUBMISSION_DIR / "hgb_gps_slim80_personal_blend_prob.csv"
