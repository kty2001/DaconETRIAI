...
- LGBM Optuna 30 trials + XGBoost Optuna 30 trials (타깃별 독립 탐색)
- 두 모델 예측 확률 단순 평균
- mUsageStats 전체 제거 (원본 lgbm_optuna_prob 0.6178 기준 재현)
출력: submission/lgbm_xgb_ensemble_prob.csv (clip 0.1~0.9)
"""
...
# mUsageStats 전체 제거 (원본 0.6178 재현)
DROP_USAGE = [
...
def train_and_predict(train, test, parquet_feat):
    ...
    n_feat = list(fold_by_target[TARGETS[0]][0][2].shape[1:2])
    ...
    print(f"=== Phase 1-B: XGBoost Optuna ({N_TRIALS} trials/target) ===")
    best_xgb = {}
    for t in TARGETS:
        y = train[t].values
        study = optuna.create_study(direction="minimize",
                                    sampler=optuna.samplers.TPESampler(seed=42))
        study.optimize(make_objective("xgb", XGB_SEARCH, XGB_FIXED,
                                      fold_by_target[t], y),
                       n_trials=N_TRIALS, show_progress_bar=False)
    ...
    print(f"=== Phase 2: LGBM+XGB 앙상블 ({n_seeds} seeds) ===")
    ...
    for seed_i, seed in enumerate(SEEDS):
        ...
        seed_xgb_oof   = {t: np.zeros(len(train)) for t in TARGETS}
        seed_xgb_test  = {t: np.zeros(len(test))  for t in TARGETS}

        for t in TARGETS:
            y = train[t].values

            lgbm_params = {**best_lgbm[t], "random_state": seed,
                           "subsample": best_lgbm[t].get("subsample", 0.8),
                           "colsample_bytree": best_lgbm[t].get("colsample_bytree", 0.8)}
            xgb_params  = {**best_xgb[t],  "random_state": seed,
                           "subsample": best_xgb[t].get("subsample", 0.8),
                           "colsample_bytree": best_xgb[t].get("colsample_bytree", 0.8)}
            ...
            for tr_idx, val_idx, X_tr, X_val in fold_by_target[t]:
                ...
                xm.fit(X_tr, y[tr_idx])
                seed_xgb_oof[t][val_idx]  = xm.predict_proba(X_val)[:, 1]
                seed_xgb_test[t]         += xm.predict_proba(te_by_target[t])[:, 1] / cv.n_splits

        for t in TARGETS:
            lgbm_oof[t]  += seed_lgbm_oof[t]  / n_seeds
            lgbm_test[t] += seed_lgbm_test[t] / n_seeds
            xgb_oof[t]   += seed_xgb_oof[t]   / n_seeds
            xgb_test[t]  += seed_xgb_test[t]  / n_seeds

        # 현재 시드까지 앙상블 평균 OOF 확인
        f1s, lls = [], []
        for t in TARGETS:
            cur_lgbm = lgbm_oof[t] * n_seeds / (seed_i + 1)
            cur_xgb  = xgb_oof[t]  * n_seeds / (seed_i + 1)
            cur_ens  = (cur_lgbm + cur_xgb) / 2
    ...
    # 최종 앙상블
    ens_oof  = {t: (lgbm_oof[t]  + xgb_oof[t])  / 2 for t in TARGETS}
    ens_test = {t: (lgbm_test[t] + xgb_test[t]) / 2 for t in TARGETS}

    print()
    print(f"{'타깃':<5}  {'앙상블 F1':>8}  {'앙상블 LL':>10}  {'LGBM LL':>8}  {'XGB LL':>7}  {'기준 F1':>7}")
    print("-" * 58)
    for t in TARGETS:
        ...
        xgb_ll  = log_loss(train[t].values, xgb_oof[t])
        base    = BASELINE_F1[t]
        print(f"{t:<5}  {ens_f1:>8.3f}  {ens_ll:>10.4f}  {lgbm_ll:>8.4f}  {xgb_ll:>7.4f}  {base:>7.3f}")
...
def main():
    ...
    print(f"=== LGBM + XGBoost 앙상블 (각 {N_TRIALS} trials, 피처 {feat_count}개) ===\n")
