"""
LightGBM + CatBoost 앙상블 v2
- label 피처 확장: roll21/28 (3~4주 이동평균) + rollstd7/14 (변동성) 추가
- LGBM Optuna 30 trials + CatBoost Optuna 30 trials (타깃별 독립 탐색)
- 두 모델 예측 확률 단순 평균
- mUsageStats 전체 제거
출력: submission/lgbm_catboost_ensemble_v2_prob.csv (clip 0.1~0.9)
"""
...
def train_and_predict(train, test, parquet_feat):
    ...
    print(f"=== Phase 2: LGBM+CatBoost 앙상블 v2 ({n_seeds} seeds) ===")
...
def main():
    ...
    print(f"=== LGBM + CatBoost 앙상블 v2 ({N_TRIALS} trials) ===")
