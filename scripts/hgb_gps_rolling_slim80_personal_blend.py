"""
HGB GPS + Rolling Slim 80% Personal Blend

hgb_gps_slim80_personal_blend.py 대비 변경:
  - rolling_features.build_rolling_features() 피처 추가
    (기준 19개 피처의 3/7일 MA + delta3 = 최대 57개)
  - HGB NaN 네이티브 처리 -> rolling 초기 NaN 그대로 활용
  - 새 Optuna 키: hgb_gps_rolling_slim80_ws

출력:
  submission/optuna_params.json -> hgb_gps_rolling_slim80_ws 키
  submission/hgb_gps_rolling_slim80_personal_blend_prob.csv
"""
...
HGB_KEY_OUT  = "hgb_gps_rolling_slim80_ws"
IMP_COVERAGE = 0.80
...
def main():
    ...
    print("=== parquet v2 + GPS + Rolling feature build ===", flush=True)
    parquet_feat = build_parquet_features()
    gps_feat     = build_gps()
    parquet_feat = parquet_feat.merge(gps_feat, on=["subject_id", "date"], how="left")

    rolling_feat = build_rolling_features(parquet_feat)
    parquet_feat = parquet_feat.merge(rolling_feat, on=["subject_id", "date"], how="left")
    print(f"  rolling merged: {parquet_feat.shape[1]} columns total", flush=True)
    print()
    ...
    # Phase 0: Feature Importance (LOSO 1 seed, ET for importance)
    print("\n=== Phase 0: Feature Importance (LOSO 1 seed) ===", flush=True)
    ...
    for tr_idx, val_idx in fold_indices:
        ...
        for t in TARGETS:
            ...
            model = ExtraTreesClassifier(
                n_estimators=100, max_depth=None, max_features=0.2,
                criterion="entropy", random_state=WS_SEED, n_jobs=-1
            )
    ...
    rolling_cols = [c for c in rolling_feat.columns if c not in ("subject_id", "date")]
    selected_rolling = [c for c in rolling_cols if c in keep_feats]
    print(f"  rolling selected: {len(selected_rolling)}/{len(rolling_cols)}", flush=True)
    ...
    # Phase 3: HGB Optuna (WS OOF, checkpoint support)
    print(f"\n=== Phase 3: HGB Global Optuna ({N_TRIALS} trials/target) ===", flush=True)

    existing = load_params(HGB_KEY_OUT) or {}
    best_hgb_params = dict(existing)
    skipped = [t for t in TARGETS if t in best_hgb_params]
    if skipped:
        print(f"  checkpoint restore: {skipped} skipped")

    for t in TARGETS:
        if t in best_hgb_params:
        ...
        def objective(trial, _t=t, _cols=cols_t):
            ...
            for sid in subjects:
                ...
                model = HistGradientBoostingClassifier(**params, random_state=WS_SEED)
                model.fit(X_tr_t, y_tr)
                preds = model.predict_proba(X_val_t)[:, 1]
    ...
    for t in TARGETS:
        ...
        for sid in subjects:
            ...
            if len(np.unique(y_tr)) < 2:
                ...
                continue
            model = HistGradientBoostingClassifier(**best_hgb_params[t], random_state=WS_SEED)
            model.fit(X_tr_t, y_tr)
            preds = model.predict_proba(X_val_t)[:, 1]
            all_y_true.extend(ws_cache[sid]["y_val"][t].tolist())
            all_y_pred.extend(np.clip(preds, 1e-6, 1 - 1e-6).tolist())
    ...
    print(f"  avg WS OOF LL: {np.mean(list(ws_oof_lls.values())):.4f}", flush=True)

    # Phase 5: Full data Global HGB (10 seeds)
    print(f"\n=== Phase 5: Global HGB model ({len(SEEDS)} seeds) ===", flush=True)
    hgb_test_global = {t: np.zeros(len(sample)) for t in TARGETS}
    for seed in SEEDS:
        for t in TARGETS:
            drop_cols = [f"subj_mean_{t}"] + DROP_USAGE
            ...
            model = HistGradientBoostingClassifier(**best_hgb_params[t], random_state=seed)
            model.fit(X_all_t, train_reset[t].values)
            hgb_test_global[t] += model.predict_proba(X_te_t)[:, 1] / len(SEEDS)
        print(f"  seed {seed} done", flush=True)

    # Phase 6: WS OOF collection (HGB global + ET personal)
    print(f"\n=== Phase 6: WS OOF collection (HGB global + ET personal) ===", flush=True)
    ...
    for sid in subjects:
        ...
        for t in TARGETS:
            ...
            if len(np.unique(y_tr)) >= 2:
                glb = HistGradientBoostingClassifier(**best_hgb_params[t], random_state=WS_SEED)
                glb.fit(X_tr_t, y_tr)
                glb_preds = glb.predict_proba(X_val_t)[:, 1]
            else:
                glb_preds = np.full(len(val_pos), float(y_tr.mean()))

            X_pers_raw  = X_tr_t.iloc[sid_mask_in_tr]
            y_pers      = y_tr[sid_mask_in_tr]
            tr_med_pers = X_pers_raw.median()
            X_pers_f    = X_pers_raw.fillna(tr_med_pers)
            X_val_f     = X_val_t.fillna(tr_med_pers)
    ...
    print(f"  global only (alpha=0.0): {blend_logloss(0.0):.4f}", flush=True)
    ...
    for t in TARGETS:
        ...
        g_v = np.array([ws_oof_global[t][p] for p in all_val_pos])
        p_v = np.array([ws_oof_pers[t][p]   for p in all_val_pos])
        bl  = np.clip(blend_probs(g_v, p_v, best_alpha), 1e-6, 1 - 1e-6)
        print(f"    {t}: {log_loss(y_v, bl):.4f}", flush=True)

    # Phase 8: Personal ET models (10 seeds)
    print(f"\n=== Phase 8: Personal ET models ({len(SEEDS)} seeds) ===", flush=True)
    hgb_test_pers = {t: np.zeros(len(sample)) for t in TARGETS}
    for sid in subjects:
        ...
        for t in TARGETS:
            ...
            hgb_test_pers[t][mask_te] = preds_sid
    ...
    hgb_test_blend = {t: blend_probs(hgb_test_global[t], hgb_test_pers[t], best_alpha)
                      for t in TARGETS}
    ...
    hgb_test_cal = {t: hgb_test_blend[t].copy() for t in TARGETS}
    for sid in subjects:
        mask_te = (sample["subject_id"] == sid).values
        if mask_te.sum() == 0:
            continue
        for t in TARGETS:
            hgb_test_cal[t][mask_te] = apply_logit_bias(
                hgb_test_blend[t][mask_te], biases[sid][t]
            )

    result_prob = sample[["subject_id", "sleep_date", "lifelog_date"]].copy()
    for t in TARGETS:
        result_prob[t] = hgb_test_cal[t].clip(0.1, 0.9)
    ...
    out_path = SUBMISSION_DIR / "hgb_gps_rolling_slim80_personal_blend_prob.csv"
    result_prob.to_csv(out_path, index=False)
    print(f"\nsaved: {out_path}")
    print(f"WS OOF blend LL: {best_ll:.4f}  (alpha={best_alpha:.4f})")
    print("비교: hgb_gps_slim80_personal_blend WS OOF=0.6249")
