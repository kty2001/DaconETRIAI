...
| 6 | **3모델 앙상블 (LGBM+CAT+기타)** | 다양성 확대 | 미시도 — 후보 분석 완료 |
| 7 | **clip 범위 최적화** | [0.1,0.9] 대신 [0.05,0.95]·[0.15,0.85] 등 실험 | 미시도 |

#### 3번째 모델 후보 분석

현재 앙상블(LGBM leaf-wise + CatBoost symmetric tree)은 모두 트리 기반 부스팅 계열. 다양성 확대를 위한 후보:

| 후보 | 특징 | LOSO 적합성 | 다양성 |
|------|------|:-----------:|:------:|
| **ExtraTrees** | 랜덤 임계값 분할 + bagging | ★★★★★ 과적합 최소 | ★★★★★ 부스팅과 완전 다름 |
| **Logistic Regression** | 선형 모델 | ★★★★ 고편향 저분산 | ★★★★★ 완전 다른 계열 |
| **Random Forest** | bagging + 최적 분할 | ★★★★ | ★★★★ 부스팅과 다름 |
| **HistGradientBoosting** | sklearn의 LGBM 유사 | ★★★ | ★★ LGBM과 거의 유사 |

> **1순위 ExtraTrees**: random split threshold → 부스팅 계열과 가장 다른 결정 경계. LOSO 소규모 fold(~45행)에서 iteration 반복 없음 → 과적합 최소. XGBoost는 이미 실패 확인되어 제외.
